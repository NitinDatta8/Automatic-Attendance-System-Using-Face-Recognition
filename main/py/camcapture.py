...
for image_file in os.listdir("{}/assets/test".format(curd)):
    full_file_path = os.path.join("{}/assests/test".format(curd), image_file)
    cv2.imshow(img_file)
    print("Looking for faces in {}".format(image_file))
    frame = cv2.imread(full_file_path,-1)

    predictions = predict(frame, model_path="{}/assets/models/trained_knn_model.clf".format(curd))
...
attendance_final
attendance_final.to_csv("/assets/attendance.csv")
